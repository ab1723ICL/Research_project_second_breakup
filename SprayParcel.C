...
)
{
    ...
    this->sigma_s() = this->sigma() - pow((this->Qdens()*pi/6.0*pow(this->d0(), 3.0)),2)/(8.0*constant::electromagnetic::epsilon0.value()*pi*pi*pow(this->d(),3.0));
    
    //std::ofstream outputFile("output.txt", std::ios::out);
    //ofstream << "Sigma_s" << sigma_s << endl;
    //outputFile << "Sigma_s = " << sigma_s << std::endl;
    //Info <<"sigma_s_SprayParcel.C:" << this->sigma_s() <<endl;
    Info << "Qdens_SprayParcel.C:" << this->Qdens() << endl;
    Info << "d0_SprayParcel.C:" << this->d0() << endl;

    scalar netChargeParent = this->Qdens()*pow(this->d0(),3.0)*pi/6.0;
    Info << "netChargeparent_sprayParcel:" << netChargeParent << endl;
    this->netChargeChild() = (this->nParticle()/this->nParticle())*netChargeParent;
    Info << "netChargeChild_sprayParcel:" << this->netChargeChild() << endl;
    //QdensChild = netChargeChild/(pow(d,3.0)*pi/6.0);
    //this->Qdens() = this->netChargeChild()/(pow(this->d(),3.0)*pi/6.0);
    //Info << "Qdens_sprayParcel2:" << this->Qdens() << endl;

    //scalar Qdens_old = this->Qdens();
    //scalar dParent_old = this->d();
    //scalar nParticle_old = this->nParticle();
    scalar parcelMassChild = 0.0;
    scalar dChild = 0.0;
    scalar QdensChild =0.0;
    if
    (
    ...
    )
    {
        ...
        Info << "Child characteristics after breakup : " << dChild << "; --- " << parcelMassChild << endl; // to be removed

	// Add child parcel as copy of parent
        SprayParcel<ParcelType>* child = new SprayParcel<ParcelType>(*this);

        //scalar m=1.8; // Shrimpton, "Pulsed charged sprays: application to DISI engines during early injection" 2003
        //scalar netChargeChild = (parcelMassChild/child->mass0())*(this->Qdens() * pow(this->d0(),3.0)*pi/6.0)*pow(dChild/dParent_old,m);
        //scalar netChargeParent = this->Qdens()*pow(this->d0(),3.0)*pi/6.0 - netChargeChild;
        //scalar QdensChild = netChargeChild/((pow(dChild,3.0)*pi/6.0)*parcelMassChild/child->mass0());

        //scalar QdensChild = this->Qdens * pow(dChild/dParent_old,m);
        //scalar netChargeChild = QdensChild*pow(dChild,3.0)*pi/6.0*parcelMassChild/child->mass0();
        //scalar netChargeParent = this->Qdens()*pow(this->d0,3.0)*pi/6.0 - netChargeChild;
        ...
        //scalar netChargeParent = this->Qdens()*pow(dParent_old,3.0)*pi/6.0;
        //scalar netChargeChild = (nParticle_old/child->nParticle())*netChargeParent;
        //QdensChild = netChargeChild/(pow(this->d(),3.0)*pi/6.0);
        //this->Qdens() = netChargeChild/(pow(this->d(),3.0)*pi/6.0);
        this->d0() = this->d();
        ...
        child->Qdens() = QdensChild;
        //child->Qdens0() = QdensChild;
...
)
{
    ...
    //this->sigma_s() = sigma_ - pow((this->Qdens()*pi/6.0*pow(this->d0_, 3.0)),2)/(8.0*constant::electromagnetic::epsilon0*pi*pi*pow(this->d(),3.0));

    // Inverse of characteristic viscous damping time
    scalar rtd = 0.5*TABCmu*mu_/(this->rho()*r2);

    // Oscillation frequency (squared)
    scalar omega2 = TABComega*this->sigma_s()/(this->rho()*r3) - rtd*rtd;

    if (omega2 > 0)
    {
        scalar omega = sqrt(omega2);
        scalar We =
            this->We(td.rhoc(), this->U(), td.Uc(), r, this->sigma_s())/TABtwoWeCrit;
...
:
    ...
    sigma_s_(p.sigma_s_),
    Qdens0_(p.Qdens0_),
    netChargeChild_(p.netChargeChild_),
    mu_(p.mu_),
